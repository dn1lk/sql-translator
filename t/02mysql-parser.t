...

{
  ...
  my $f3 = shift @fields;
  is($f3->name,      'billing_address_id', 'Third field name is "billing_address_id"');
  is($f3->data_type, 'int',                'Type is "int"');
  is($f3->size,      11,                   'Size is "11"');

  my $f4 = shift @fields;
  is($f4->name,      'shipping_address_id', 'Fourth field name is "shipping_address_id"');
  is($f4->data_type, 'int',                 'Type is "int"');
  is($f4->size,      11,                    'Size is "11"');
  ...
  is(join(',', $i2->fields), 'billing_address_id', 'Index is on "billing_address_id"');

  my $i3 = shift @indices;
  is($i3->type,              NORMAL,                'Third index is normal');
  is(join(',', $i3->fields), 'shipping_address_id', 'Index is on "shipping_address_id"');

  my $i4 = shift @indices;
  is($i4->type,              NORMAL,               'Third index is normal');
  is(join(',', $i4->fields), 'member_id,store_id', 'Index is on "member_id,store_id"');
  ...
  is(join(',', $c3->fields),           'billing_address_id', 'Constraint is on "billing_address_id"');
  is($c3->reference_table,             'address',            'To table "address"');
  is(join(',', $c3->reference_fields), 'address_id',         'To field "address_id"');

  my $c4 = shift @constraints;
  is($c4->type,                        FOREIGN_KEY,           'Constraint is a FK');
  is(join(',', $c4->fields),           'shipping_address_id', 'Constraint is on "shipping_address_id"');
  is($c4->reference_table,             'address',             'To table "address"');
  is(join(',', $c4->reference_fields), 'address_id',          'To field "address_id"');
  ...
  is(join(',', map { $_ || '' } $c5->reference_fields), '',          'No reference fields defined');
...
#
{
  ...
  is_deeply($t1f2->default_value,      \'CURRENT_TIMESTAMP', 'Field has right default value');
  ...
  is_deeply($t1f3->default_value, \'CURRENT_TIMESTAMP', 'Field has right default value');
  ...
  is(
    join(',', $view1->fields),
    join(
      ',', qw[ asset_id fq_name folder_name asset_name annotation
  ...
  is_deeply(
    \@options,
    [ 'ALGORITHM=UNDEFINED', 'DEFINER=`cmdomain`@`localhost`', 'SQL SECURITY DEFINER', ],
...

{
  ...
  for my $t_option_ref ($table->options) {
    ...
    if ($key eq 'ENGINE') {
      ...
      is_deeply($value, [ 'sometable_0', 'sometable_1', 'sometable_2' ], "UNION option has correct set");
      $unionFound = 1;
    }
  }
...
# however in mysql 5.x (and probably other previous versions) still actually outputs that as ''
  ...
  is(
    $f3->default_value,
    "test single quotes escaped like you're",
    "Single quote in default value is unescaped properly"
  );
  is($f3->is_primary_key, 0, 'Field is not PK');
}
